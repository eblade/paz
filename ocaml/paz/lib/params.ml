...
let apply (p : user_params) key value =
  ...
  let trueness t =
    ...
    | "yes" -> true
    | "no" -> false
    | _ -> raise (Invalid_argument ("Not true/false " ^ key ^ " = " ^ value))
  in
...
let finalize (p : user_params) =
  ...
  let source =
    master ^ ":" ^ g p.site
    ^
    match p.revision with Some revision -> Int.to_string revision | None -> ""
  in
  {
    source;
    master;
    ...
    revision = p.revision;
    ending = (g p.addition ^ if g p.linebreak then "\n" else "");
  }

let print_params (p : user_params) =
  let print = Printf.fprintf stderr "%s = %s\n" in
  let pt name maybe =
    match maybe with Some x -> print name x | None -> print name "(unset)"
  in
  let m f maybe = match maybe with Some x -> Some (f x) | None -> None in
  pt "site" p.site;
